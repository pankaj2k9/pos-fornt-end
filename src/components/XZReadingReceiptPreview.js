...
export default class XZReadingReceiptPreview extends React.PureComponent {
  ...
  renderPrintBtn (isDayClosed, is) {
    return (
      <p className='control'>
        ...
        {
          !this.state.isDayCloseStarted &&
          <button className='button is-primary' onClick={() => { this.setState({ isDayCloseStarted: true }) }}>
            <FormattedMessage id='app.general.closeDay' />
          </button>
        }
        {
          !isDayClosed && this.state.isDayCloseStarted &&
          <button className='button is-primary' onClick={this.handleCloseDay} style={{marginRight: 10}}>
            <FormattedMessage id='app.general.closeEndOfDaySales' />
          </button>
        }
        {
          !isDayClosed && this.state.isDayCloseStarted &&
          <button className='button is-primary' onClick={() => { this.setState({ isDayCloseStarted: false }) }}>
  ...
  render () {
    ...
    const lastClosedDay = data.lastClosedDay
    let summaryData = []
    let netSales = 0
    let netSalesOdbo = 0

    data.orders.forEach(order => {
      if (lastClosedDay && order.dateOrdered <= lastClosedDay) {
        return
      }
